...
import { getMenu, saveMenu, getSettings, saveSettings, incrementAntarCount, appendCheckoutLog } from './storage';
import { createSessionToken, isValidSessionToken, sessionCookieName, sessionCookieOptions } from './session';
import { slugify, formatChosenOpsi } from './format';
import { HARI_OPTIONS } from './seed';
import { getJakartaDateKey, getJakartaTimeString } from './storeStatus';
...
export async function updateSettingsAction(formData) {
  ...
  const jamBukaRaw = String(formData.get('jamBuka') || '').trim();
  const jamTutupRaw = String(formData.get('jamTutup') || '').trim();
  const jamBuka = jamPattern.test(jamBukaRaw) ? jamBukaRaw : '';
  const jamTutup = jamPattern.test(jamTutupRaw) ? jamTutupRaw : '';
  if ((jamBuka && !jamTutup) || (!jamBuka && jamTutup)) {
    return { error: 'Jam buka dan jam tutup harus diisi berdua, atau dikosongkan berdua kalau tidak mau jadwal otomatis.' };
  }
  if (jamBuka && jamTutup && jamBuka >= jamTutup) {
    return { error: 'Jam tutup harus lebih besar dari jam buka.' };
  }
  ...
  const next = {
    ...
    metaDescription: truncate(formData.get('metaDescription'), 200),
    nomorWhatsApp,
...
// Catat "niat checkout" (isi keranjang pas tombol Checkout diklik) buat direkap ke
// Google Sheets 1x sehari lewat cron job — lihat app/api/cron/sync-sheets/route.js.
// Ini BUKAN bukti pesanan benar-benar terkirim/dikonfirmasi di WhatsApp (di luar
// jangkauan web ini sepenuhnya) — cuma sinyal tren, bukan catatan transaksi resmi.
// Ditulis ke Redis (ditunggu/awaited) sebelum pindah ke WhatsApp — sama seperti
// incrementAntarCountAction, ini cukup cepat (~puluhan ms) untuk tidak kerasa nambah
// delay. Publik (bukan requireAdmin/requireDriver) karena dipanggil dari checkout
// pelanggan biasa; semua input di-batasi panjangnya sebagai jaga-jaga.
export async function logCheckoutAction({ customerName, items, metode, note, total }) {
  try {
    const safeItems = Array.isArray(items) ? items.slice(0, 50) : [];
    const itemsSummary = safeItems
      .map((item) => {
        const opsi = formatChosenOpsi(item?.chosenOpsi);
        return `${truncate(item?.nama, 60)}${opsi ? ` (${truncate(opsi, 60)})` : ''} x${Number(item?.qty) || 0}`;
      })
      .join(', ');
    ...
    await appendCheckoutLog({
      tanggal: getJakartaDateKey(now),
      jam: getJakartaTimeString(now),
      nama: truncate(customerName, 100) || '-',
      item: truncate(itemsSummary, 800),
      metode: isDelivery ? 'Diantar' : 'Ambil Sendiri',
      alamat: isDelivery ? truncate(note, 300) : '',
      total: Number.isFinite(Number(total)) ? Math.round(Number(total)) : '',
    });

    return { ok: true };
  } catch (err) {
    return { error: err?.message || 'Gagal mencatat log checkout.' };
  }
}
