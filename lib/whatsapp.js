...
export function buildOrderMessage({ settings, items, customerName, note }) {
  ...
  // Baris soal harga/total & ambil sendiri di warung selalu ditaruh paling bawah,
  // ditulis sebagai kalimat-kalimat pendek yang sopan — bukan satu kalimat panjang.
  if (hasUnpriced) {
    if (hasPriced) {
      parts.push(`Total sementara: ${formatRupiah(total)} (item lain menyusul konfirmasi harga)`, '');
    }
    parts.push(
      'Mohon info total harganya ya, Mbak.',
