...
export function buildOrderMessage({ settings, items, customerName, note, metode }) {
  ...
  const closingLine = isDelivery
    ? 'Kalau sudah mau diantar, tolong infoin ya, Mbak.'
  ...
  if (isDelivery) {
    parts.push(`Metode: Diantar ke ${note.trim()}`, 'Nanti saya kirim Share Location WhatsApp juga ya, Mbak.');
  } else {
...
// Skema link langsung ke aplikasi WhatsApp (bukan halaman web wa.me). Dipakai
// khusus saat web ini dibuka standalone (PWA/Add to Home Screen) — di mode itu,
// iOS kadang tidak langsung lompat ke aplikasi lewat link wa.me biasa, malah
// nyangkut di halaman perantara "Chat on WhatsApp with...". Skema ini melompat
// langsung, dengan wa.me tetap dipakai sebagai cadangan kalau skema ini gagal
// (mis. WhatsApp belum terinstal) — lihat pemakaiannya di CartDrawer.js.
export function buildWhatsAppSchemeUrl(settings, message) {
  const phone = normalizePhone(settings.nomorWhatsApp);
